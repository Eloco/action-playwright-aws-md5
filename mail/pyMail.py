...
CODES = ['gbk','UTF-8', 'UTF-16', 'GB18030', 'BIG5']
...
def muti_decode(s):
    ...
    for code in CODES:
        try:
            fina=data.decode(encoding=code)
            if 'UTF-8' == code and data.startswith(UTF_8_BOM):
                return data.decode(encoding="UTF-8-SIG")
            return fina
...
class ReceiveMailDealer:
    ...
    def getToday(self):
        ...
        return self.search(None,f'(SINCE "{today}")')
    ...
    def getEmailFormat(self, num):
        ...
        if data[0] == 'OK':
            return email.message_from_string(muti_decode(data[1][0][1]))
        else:
            return "fetch error"
    ...
    def parse_attachment(self, message_part):
        ...
        if content_disposition:
            ...
            if bool(content_disposition and dispositions[0].lower() == "attachment"):
                ...
                file_data = message_part.get_payload(decode=False)
                attachment = {}
    ...
    def getMailInfo(self, num):
        ...
        for part in msg.walk():
            ...
            if attachment:
                ...
                if body is None:
                    body = ""
                body += muti_decode(part.get_payload(decode=True))

            elif part.get_content_type() == "text/html":
                if html is None:
                    html = ""
                html += muti_decode(part.get_payload(decode=True))
        return {
